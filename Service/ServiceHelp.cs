    ...
    public class ServiceHelp : IDisposable
    {
        protected string baseServiceUrl
        {
            get
            {
                ...
                string path = Path.GetDirectoryName(Assembly.GetExecutingAssembly().CodeBase) + "/Team5APIUrl_DEV.xml";
                configXml.Load(path);
                ...
                foreach (XmlNode node in addNodes)
                {
                    ...
                    {
                        ApiUrl = (node.ChildNodes[0]).InnerText;
                        break;
                    }
                }
                return ApiUrl;
        ...
        {
            BaseServiceUrl = $"{baseServiceUrl}/{routePrefix}";
        ...
        /// <returns></returns>
        public T PostApiCaller<T, K>(string path, K k) where T : class
        {
            path = BaseServiceUrl + path;
